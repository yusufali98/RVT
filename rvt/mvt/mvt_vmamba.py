...
import sys
from rvt.mvt.mvt_mamba import create_block, BiMambaWrapper
from rvt.mvt.vmamba_encoder import VSSM_RVT


class MVT_VMamba(nn.Module):
    def __init__(
        self,
        depth,
        img_size,
        ...
        self.vmamba_ssm_ratio = vmamba_ssm_ratio
        self.vmamba_ssm_conv = vmamba_ssm_conv
        self.vmamba_ssm_conv_bias = vmamba_ssm_conv_bias
        self.vmamba_forward_type = vmamba_forward_type
        self.vmamba_mlp_ratio = vmamba_mlp_ratio
        ...
        global_attn_depth = depth // 2
        
        layer_idx = 0
        use_bidirectional = None
        for _ in range(global_attn_depth):
        ...
        self.local_attn_layers = VSSM_RVT(drop_path_rate=self.vmamba_drop_rate,
                                          dims=self.vmamba_d_model,
                                          depths=[self.vmamba_depth],
                                          ssm_d_state=self.vmamba_d_state,
                                          ssm_ratio=self.vmamba_ssm_ratio,
    ...
    def forward(
        ...
        **kwargs,
        ...
        x = self.fc_bef_attn(ins)               # Here img_feats+proprio_feats dim = 128 * 2 is converted to 512 (mamba_d_state)
        ...
        if self.self_cross_ver == 1:
            ...
            imgx = imgx.reshape(bs * num_img, -1, num_pat_img, num_pat_img)       #  channel-wise
            ...
            imgx = self.local_attn_layers(imgx)
